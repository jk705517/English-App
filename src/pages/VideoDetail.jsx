...
const VideoDetail = ({ isDemo = false, demoEpisode = 104 }) => {
    ...
    // 默认开：没动过 toggle 的用户开箱即用顺序播放；显式关过的（localStorage='false'）保留偏好
    const [isAutoPlayNext, setIsAutoPlayNext] = useState(() => {
        const saved = localStorage.getItem('bbEnglish_isAutoPlayNext');
    ...
    const handlePrevDictation = () => {
        if (!videoData?.transcript || dictationIndex <= 0) return;
        const prevIndex = dictationIndex - 1;
        setIsSeeking(true);
        setDictationIndex(prevIndex);
        setHasPlayedCurrent(false);
        const prevTime = videoData.transcript[prevIndex].start;
    ...
    return (
        <div className="min-h-screen bg-[#f5f5f5] dark:bg-gray-900 xl:h-screen xl:flex xl:flex-col">
            ...
            {vocabPopup && (() => {
                ...
                const popupVocabId = vocabPopup.item.id !== undefined && vocabPopup.item.id !== null
                    ? vocabPopup.item.id
                    : `${videoData.id}-vocab-${vocabPopup.index}`;
                const popupIsFav = favoriteVocabIds.some(fid => String(fid) === String(popupVocabId));
                return (
                    <div
                            ...
                            <div className="relative px-5 pt-4 pb-4" style={{ backgroundColor: pc.bg }}>
                                ...
                                <div className="absolute right-3 top-3 flex items-center gap-0.5">
                                    <button
                                        onClick={() => {
                                            if (!user && !isDemo) { alert('登录后才能使用本子功能'); return; }
                                            setNotebookDialogItem({ itemType: 'vocab', itemId: popupVocabId, videoId: Number(videoData.id) });
                                            setNotebookDialogOpen(true);
                                        }}
                                    ...
                                    <button
                                        onClick={() => handleToggleVocabFavorite(popupVocabId)}
                                        className="w-8 h-8 flex items-center justify-center rounded-full transition-colors"
                                        style={{ color: pc.text, backgroundColor: 'transparent' }}
                                        onMouseEnter={e => e.currentTarget.style.backgroundColor = 'rgba(0,0,0,0.08)'}
                                        onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}
                                        title={popupIsFav ? '取消收藏' : '收藏'}
                                    >
                                ...
                                {/* 单词标题 + 类型标签 */}
                                <div className="flex items-center gap-2 flex-wrap pr-28">
